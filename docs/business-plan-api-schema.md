...
- `GET /business-plans?month=YYYY-MM` — one-time events overlapping that month; yearly recurring with anchor in that month.
- `GET /business-plans?year=YYYY` — all events with any occurrence in that year (frontend groups into 12 month tiles via `groupEventsByMonth`).
...
When `GET /business-plans?month=YYYY-MM` or `?year=YYYY`:
...
```http
PUT /business-plans/{id}/yearly-amounts/{year}
```
...
Used by the business plan **monthly** and **yearly** calendar views. Send **either** `month` **or** `year`, not both.
...
```http
GET /business-plans?type=all&month=2026-05
GET /business-plans?type=district&year=2026&districtId=674a00000000000000000001
```
...
```javascript
db.business_plans.createIndex({ startDate: 1 });
db.business_plans.createIndex({ startDate: 1 });
db.business_plans.createIndex({ startDate: 1, belongsTo: 1 });
db.business_plans.createIndex({ districtId: 1, startDate: 1 });
db.business_plans.createIndex({ departmentId: 1, startDate: 1 });
```
...
`GET /business-plans` may include a `stats` object so the client does not recompute:
...
- Opens from **List** button on the business plan page.
...
| Event wizard (modal, 8 steps) | `src/dashboard/owner/buisnessplan/EventWizard.jsx` |
| Event utils (recurrence + calendar grouping) | `src/utils/businessPlanEvent.js` |
| Constants | `src/constants/businessPlan.js` |
| Calendar month / pills | `src/dashboard/owner/buisnessplan/CalendarMonth.jsx` |
| Calendar year grid | `src/dashboard/owner/buisnessplan/CalendarYear.jsx` |
| Calendar type statistics | `src/dashboard/owner/buisnessplan/BusinessPlanTypeStats.jsx` |
| Month/year picker | `src/dashboard/owner/buisnessplan/CalendarPeriodPicker.jsx` |
| Calendar date helpers | `src/utils/calendar.js` |
| List drawer | `src/dashboard/owner/buisnessplan/BusinessPlanListDrawer.jsx` |
| Add venue modal | `src/dashboard/owner/buisnessplan/AddVenueModal.jsx` |
| Amount helpers | `src/utils/businessPlanAmounts.js` |
...
| 2026-05-28 | Monthly/yearly calendar toggle; `GET /business-plans?year=YYYY`; period picker (month+year or year only) |
